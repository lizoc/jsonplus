...
- [Syntax](#syntax)
  ...
  - [Comments](#comments)
  - [Optional root braces](#optional-root-braces)  
  ...
  - [Substitutions](#substitutions)
    ...
    - [The `+=` member separator](#the-plus-equal-member-separator)
  ...
  - [API Recommendations](#api-recommendations)
    ...
    - [Object merging and file merging](#object-merging-and-file-merging)
...
Definitions
-----------
 - a _key_ is a string JSON would have to the left of `:` and a _value_ is anything JSON would have to the right of `:`. i.e. the two halves of an object _member_.
 - a _value_ is any "value" as defined in the JSON spec, plus unquoted strings and substitutions as defined in this spec.
 - a _literal value_ is any value excluding object or array.
 - a _member_ is a key, any separator such as ':', and a value.
...
A Json+ file is invalid if it omits the opening `{` but still has a closing `}`; the curly braces must be balanced. This behavior is analogous to XML, as opposed to HTML.

### Key-value separator
In addition to the colon `:`, Json+ supports using the equal `=` character as well, i.e. to separate keys from values.

If a key is followed by `{`, the `:` or `=` may be omitted. So `"foo" {}` means `"foo" : {}`

### Commas
Values in arrays, and members in objects, need not have a comma between them as long as they have at least one ASCII newline (`\n`, decimal value 10) between them.

The last item in an array or last member in an object may be followed by a single comma. This extra comma is ignored.
...
### Value concatenation
The value of an object member or array item may consist of multiple values which are combined. There are three kinds of value concatenation:
...
String value concatenation is allowed in member keys, in addition to member values and array items. Objects and arrays do not make sense as member keys.
...
Arrays allow you to use newlines instead of commas, but not whitespace instead of commas. Non-newline whitespace will produce concatenation rather than separate items.
...
- a single _quoted_ string which is interpreted heuristically as URL, filename, or library resource. 
- `url()`, `file()`, or `assembly()` surrounding a quoted string which is then interpreted as a URL, file, or library resource. The string must be quoted, unlike in CSS.  
- `require()` surrounding one of the above
...
Instead, the next value must be a _quoted_ string or a quoted string surrounded by `url()`, `file()`, or `assembly()`. This value is the _resource name_.

Together, the unquoted `include` and the resource name substitute for an object member syntactically, and are separated from the following object members or includes by the usual comma (and as usual the comma may be omitted if there's a newline).

If an unquoted `include` at the start of a key is followed by anything other than a single quoted string or the `url("")`/`file("")`/`assembly("")` syntax, it is invalid and an error should be generated.
...
If however an included resource is mandatory then the name of the included resource may be wrapped with `require()`, in which case file parsing will fail with an error if the resource cannot be resolved.

The syntax for this can be:

```
include require("foo.conf")
include require(file("foo.conf"))
include require(assembly("foo.conf"))
include require(url("http://localhost/foo.conf"))
```
...
A quoted string not surrounded by `url()`, `file()`, `assembly()` must be interpreted heuristically. The heuristic is to treat the quoted string as:
...
---------------------------------------------------
In some file formats and contexts, there isn't a good way to define arrays. To provide some mechanism for this, implementations should support converting objects with numeric keys into arrays. For example, this object:
...
This conversion should:
 ...
 - be done lazily when required to avoid a type error, NOT eagerly anytime an object has numeric keys.  
 - be done when you would do an automatic type conversion (see the section "Automatic type conversions" below).  
 - be done in a concatenation when a list is expected and an object with numeric keys is found.  
 - not occur if the object is empty or has no keys which parse as positive integers.  
 - ignore any keys which do not parse as positive integers.  
...
This rule for merging objects loaded from different files is _exactly_ the same behavior as for merging duplicate members in the same file. All merging works the same way.
